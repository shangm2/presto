            ...
            throws RuntimeException
    {
        try {
            if (response.getStatusCode() != 200) {
                throw new PrestoException(NATIVE_EXECUTION_TASK_ERROR, "Thrift request failed with response code " + response.getStatusCode());
            }

            byte[] responseBody = readResponseBytes(response);

            if (responseBody == null || responseBody.length == 0) {
                throw new PrestoException(NATIVE_EXECUTION_TASK_ERROR, "Thrift request receives empty response");
            }
            ...
            throw new PrestoException(NATIVE_EXECUTION_TASK_ERROR, "Failed to deserialize thrift response", e);
        }
    }
