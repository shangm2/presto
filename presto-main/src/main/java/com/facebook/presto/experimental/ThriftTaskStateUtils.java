...
public class ThriftTaskStateUtils
{
    ...
    static {
        log.info(format("ThriftTaskStateUtils with code_to_task_state: %s.", CODE_TO_TASK_STATE));
    }

    public static boolean isDone(ThriftTaskState state)
    {
        boolean result = TERMINAL_TASK_STATES.contains(state);
        log.info(format("ThriftTaskStateUtils: %s with result %s", state, result));
        return result;
    }
