        ...
        implements RemoteTask
{
    ...
    private volatile ExecutorService taskExecutorService;
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(this::start);
            return;
        }
        try (SetThreadName ignored = new SetThreadName("HttpRemoteTask-%s", taskId)) {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(() -> addSplits(splitsBySource));
            return;
        }
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(() -> noMoreSplits(sourceId));
            return;
        }
        taskExecutorService.execute(() -> {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(() -> noMoreSplits(sourceId, lifespan));
            return;
        }
        taskExecutorService.execute(() -> {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(() -> setOutputBuffers(newOutputBuffers));
            return;
        }
        taskExecutorService.execute(() -> {
            if (getTaskStatus().getState().isDone()) {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(() -> whenSplitQueueHasSpace(weightThreshold));
            return null;
        }
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(this::updateSplitQueueSpace);
            return;
        }
        taskExecutorService.execute(() -> {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(() -> processTaskUpdate(newValue, sources));
            return;
        }
        taskExecutorService.execute(() -> {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(this::scheduleUpdate);
            return;
        }
        taskExecutorService.execute(() -> {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(this::sendUpdate);
            return;
        }
        taskExecutorService.execute(() -> {
            TaskStatus taskStatus = getTaskStatus();
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(this::cancel);
            return;
        }
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(this::cleanUpTask);
            return;
        }
        taskExecutorService.execute(() -> {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(this::abort);
            return;
        }
        taskExecutorService.execute(() -> {
            if (getTaskStatus().getState().isDone()) {
    ...
    {
        if (!initialized.get()) {
            pendingActions.offer(() -> abort(status));
            return;
        }
        taskExecutorService.execute(() -> {
