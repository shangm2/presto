        ...
        extends AbstractTypedThriftCodec<ConnectorTransactionHandle>
{
    ...
    {
        return connectorThriftCodecManager.getConnectorTransactionHandleThriftCodec(connectorId).map(codec -> codec.deserialize(reader)).orElse(null);
    }

    @Override
    public void writeConcreteValue(String connectorId, ConnectorTransactionHandle value, TProtocolWriter writer)
    {
        connectorThriftCodecManager.getConnectorTransactionHandleThriftCodec(connectorId).ifPresent(codec -> codec.serialize(value, writer));
    }
