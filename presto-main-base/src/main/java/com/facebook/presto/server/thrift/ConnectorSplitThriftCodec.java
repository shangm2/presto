        ...
        extends AbstractTypedThriftCodec<ConnectorSplit>
{
    ...
    {
        return connectorThriftCodecManager.getConnectorSplitThriftCodec(connectorId).map(codec -> codec.deserialize(reader)).orElse(null);
    }

    @Override
    public void writeConcreteValue(String connectorId, ConnectorSplit value, TProtocolWriter writer)
    {
        connectorThriftCodecManager.getConnectorSplitThriftCodec(connectorId).ifPresent(codec -> codec.serialize(value, writer));
    }
